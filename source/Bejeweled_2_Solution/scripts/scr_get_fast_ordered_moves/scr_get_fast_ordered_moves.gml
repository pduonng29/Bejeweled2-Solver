function scr_get_fast_ordered_moves(board, prev_move, max_take) {
    ...
    for (var i = 0; i < array_length(raw_moves); i++) {
        ...
        var sc = scr_fast_score_move(board, mv);
    ...
    while (array_length(scored) > 0 && array_length(result) < max_take) {
        var best_i = 0;
        var best_score = scored[0].node_score;

        for (var j = 1; j < array_length(scored); j++) {
            if (scored[j].node_score > best_score) {
                best_score = scored[j].node_score;
                best_i = j;
            }
        }
